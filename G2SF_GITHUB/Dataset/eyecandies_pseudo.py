...
def normal_augmentation(class_name):
    augmentor = T.Compose([T.RandomRotation(degrees=(-90, 90)), T.RandomHorizontalFlip(), T.RandomVerticalFlip(),
                           T.RandomAffine(degrees=0, translate=(0.025, 0.025), scale=(0.9, 1.1), shear=0)])
    return augmentor


class EyeCandiesPseudoDataset(BaseDataset):
    ...
    def random_perlin_mask(self, fg_mask, reduce_threshold):
    ...
    def generate_pseudo_anomaly(self, img_path, pcd_path):
        ...
        source_name = self.anomaly_source[source_idx]
        if 'Cane' in source_name or 'Loll' in source_name:
            reduce_threshold = True
        else:
        ...
        a_fg_mask = source_mask

        a_mask = self.random_perlin_mask(a_fg_mask, reduce_threshold)
        ...
        """ Since we may paste some anomalies on the background, we need to exclude them
        """
        n_img[n_fg_mask == 0] = 0
        n_pcd[n_fg_mask == 0] = 0
        final_mask[n_fg_mask == 0] = 0
